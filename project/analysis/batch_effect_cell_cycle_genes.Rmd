...
```{r}
clust_vec <- 2:7
if(file.exists("../rdas/batch_effect_cell_cycle_genes.rda")) {
    load("../rdas/batch_effect_cell_cycle_genes.rda")
} else {
    exp_batch_removed_cpm_data <- round(100*exp(batch_removed_cpm))
    topics_list <- lapply(clust_vec, function(per_clust) {
        topics(exp_batch_removed_cpm_data, K = per_clust, tol=0.005)
        })    
    names(topics_list) <- paste0("clust",clust_vec)
    save(topics_list, file = "../rdas/batch_effect_cell_cycle_genes.rda")
}
```
