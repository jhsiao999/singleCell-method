...
The main focus of this script is to perform a clustering of iPSC using Structure and then trying to figure out if that clustering corresponds to the cell cycle phases as obtained by John Blischak. We first use his code to filter out the data set for both reads and molecules so as to remove the "drop-outs" cells and genes that are not expressed.
...
K=6;
Topic_Clus_reads <- topics(t(reads_single),K=6,tol=0.003);

write.table(Topic_Clus_reads$omega,paste0('../data_derived/topic_omega_reads_clus_',K,'.txt'));

write.table(Topic_Clus_reads$theta,paste0('../data_derived/topic_theta_reads_clus_',K,'.txt'));


```

## Cell cycle effects (reads)
