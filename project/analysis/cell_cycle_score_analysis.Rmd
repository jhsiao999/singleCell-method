...
In this script, we try to investigate the behavioral pattern of the cell phase scores for the 5 different cell cycle phases for the single cell seq data collected by Yoav's lab.
...
The iplotCurves for cell cycle genes corresponding to first phase.
...
We use these cell cycle genes to get the cell cycle scores for each cell.
...
We perform a **iplotCurves** plot for the cell phase scores of the `r dim(molecules_single)[2] ` single cells.
